package sm.mobile.jx.core.map
{
	...
	public class MBTilesUtilEx extends Object
	{
		...
		public function getTile(tableName:String,row:int,rowName:String, col:int,colName:String, level:Number,levelName:String,tileDataName:String) : ByteArray
		{
			var result:SQLResult;
			var data:Array;
			...
			if (_bytefromIsAS)
			{
				sql = "SELECT "+ tileDataName +" FROM "+tableName  + " where "+rowName +" = :tile_row AND "+ colName +" = :tile_column AND "+levelName+" = :level";
			}
			else
			{
				sql = "SELECT CAST("+ tileDataName+" AS ByteArray) AS tile_data " +" FROM "+tableName  + " where "+rowName +" = :tile_row AND "+ colName +" = :tile_column AND "+levelName+" = :level";
			}
			stmtTemp.text = sql;
			stmtTemp.parameters[":tile_row"] = row;
			stmtTemp.parameters[":tile_column"] = col;
